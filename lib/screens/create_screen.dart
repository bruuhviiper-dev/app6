...
class _CreateScreenState extends State<CreateScreen> {
  ...
  Future<void> _shareImage(bool noWatermark) async {
    ...
    try {
      ...
      final file =
          File('${dir.path}/indireta_${DateTime.now().millisecondsSinceEpoch}.png');
      await file.writeAsBytes(bytes);
      await Share.shareXFiles([XFile(file.path)],
          text: noWatermark ? '' : 'Feito no app Frases Indiretas 💬');
    } finally {
  ...
  Future<void> _saveGallery() async {
    ...
    try {
      final bytes = await _capture();
      if (bytes == null) return;
      await Gal.putImageBytes(bytes, album: 'Frases Indiretas');
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final sig = state.customSignature.isNotEmpty
        ...
        : (state.canRemoveWatermark ? '' : '💬 Frases Indiretas');
